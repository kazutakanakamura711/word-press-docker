...
| `npm run lint`       | TypeScript のLintチェック（ESLint）                      |
| `npm run lint:fix`   | TypeScript のLint自動修正                                |
| `npm run format`     | TS / CSS を Prettier で整形 → PHP を php-cs-fixer で整形 |
| `npm run format:php` | PHP ファイルのみ php-cs-fixer で整形（Docker 経由）      |
...
npm run lint        # Lintチェック
npm run lint:fix    # 自動修正
npm run format      # 全ファイル整形（Docker コンテナが起動している必要あり）
```
...
**php-cs-fixer（medical-theme のみ）**
PHP ファイルを `@PSR12` ベースのルールで整形します。Docker コンテナ内の PHP を使用するためホスト Mac に PHP のインストールは不要です。

- 設定ファイル: `medical-theme/.php-cs-fixer.dist.php`
- 実行バイナリ: `medical-theme/vendor/bin/php-cs-fixer`（Docker コンテナ内）
- ラッパースクリプト: `.vscode/php-cs-fixer-docker.sh`
...
| `array_syntax`                | 配列の短縮構文 `[]` を強制            |
| `concat_space`                | 文字列結合演算子 `.` の前後にスペース |
| `yoda_style`                  | ヨーダ記法を無効化                    |
| `trailing_comma_in_multiline` | 複数行配列の末尾カンマ                |
...
## VS Code PHP format on save（medical-theme）

`medical-theme/` の PHP ファイルを保存した際に php-cs-fixer が自動実行されます。

### 必要拡張機能

- [junstyle.php-cs-fixer](https://marketplace.visualstudio.com/items?itemName=junstyle.php-cs-fixer) をインストールしてください

### 設定

`.vscode/settings.json` にプロジェクト専用の設定が含まれています。Docker コンテナ（`wordpress_app`）が起動している状態で PHP ファイルを保存すると、`.vscode/php-cs-fixer-docker.sh` 経由でコンテナ内の php-cs-fixer が実行されます。

> my-theme は引き続き Prettier が PHP フォーマッターとして機能します。
