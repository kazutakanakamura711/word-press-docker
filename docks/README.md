...
| 診療案内           | `services`                         | 科目01〜16 ごとのタイトル / 説明文 / 画像 / 詳細ページ（診療科）              |
| 診療科詳細         | `department` カスタム投稿タイプ    | 取扱い疾患 / こんな症状はお早目に受診を / 検査・治療について / 設備画像01〜06 |
...
- 各診療科の詳細ページは `department` カスタム投稿タイプで管理します（URL: `/services/[slug]/`）
- `services` ページの各科目スロットで `department` 投稿を選択すると「詳細を見る」リンクが表示されます
- 診療科詳細のヘッダー背景画像は `services` ページの該当科目に設定した画像を自動で使用します
...
3. **診療科詳細**（管理画面: 診療科）→ 各科の取扱い疾患 / 症状 / 検査・治療 / 設備画像を入力し、`services` ページの該当スロットで紐付ける
4. **トップ**（フロントページ）→ ヒーロー背景画像 / About セクション画像
5. **医院について**（スラッグ: `about`）→ 院長画像 / 氏名 / 挨拶文
